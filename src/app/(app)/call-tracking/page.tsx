...
const columns: ColumnDef<any, any>[] = [
  ...
  { accessorKey: "qualification", header: "Qualificação", cell: ({ row }) => row.original.qualification ? <Badge variant={QUAL_BADGE[row.original.qualification] || "secondary"}>{row.original.qualification}</Badge> : <span className="text-t3">—</span> },
...
export default function CallTrackingPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-t1">Call Tracking</h1>
        <p className="text-sm text-t3">Rastreie e qualifique ligações de campanhas</p>
      </div>
