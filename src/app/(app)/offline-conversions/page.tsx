...
export default function OfflineConversionsPage() {
  ...
  const handleProcess = async (uploadId: string) => {
    ...
    try {
      ...
      if (error) throw error;
      toast.success("Processamento concluido!", { description: `${data?.matchedRows || 0} de ${data?.totalRows || 0} linhas vinculadas a campanhas.` });
      queryClient.invalidateQueries({ queryKey: ["offline-uploads"] });
    } catch (err: any) {
        ...
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-t3 mt-0.5">Separador: virgula ou ponto-e-virgula. Primeira linha = header.</p>
            </div>
          </div>
        </CardContent>
      </Card>
