  ...
  return (
    <div className="space-y-6 animate-fade-up">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-heading font-bold text-t1">Metas & OKRs</h1>
          <p className="text-sm text-t3 mt-1">Acompanhe o progresso das suas metas</p>
        </div>
      ...
      {goals && goals.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {goals.map((goal: any) => {
            ...
            return (
              <Card key={goal.id} className="hover:border-primary/20 transition-all">
                <CardContent className="p-4">
                  ...
                  <div className="mb-2">
                    <div className="flex justify-between text-xs text-t3 mb-1"><span>{goal.current_value || 0} / {goal.target_value || 0}</span><span>{progress.toFixed(0)}%</span></div>
                  ...
                  {goal.goal_milestones?.slice(0, 3).map((m: any) => (
                    <div key={m.id} className="flex items-center gap-2 text-xs text-t3">
                      <CheckCircle className={cn("h-3 w-3", m.completed ? "text-success" : "text-t3/40")} />
        ...
        <Card><CardContent className="py-16 text-center"><p className="text-t3">Nenhuma meta criada.</p><Button className="mt-4" onClick={() => setCreating(true)}><Plus className="h-4 w-4 mr-2" />Criar Meta</Button></CardContent></Card>
      )}
    </div>
  );
}
