...
const columns: ColumnDef<any, any>[] = [
  { accessorKey: "keyword", header: "Keyword", cell: ({ row }) => <span className="font-medium">{row.original.keyword}</span> },
  {
    ...
    cell: ({ row }) => {
      ...
      return <span className={cn("font-mono font-bold", pos <= 3 ? "text-success" : pos <= 10 ? "text-warning" : "text-muted-foreground")}>{pos}</span>;
    },
  },
  ...
  { accessorKey: "url", header: "URL", cell: ({ row }) => <span className="text-xs text-muted-foreground truncate max-w-[200px] block">{row.original.url || "—"}</span> },
];
...
export default function SEOPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="SEO Monitor"
        description="Acompanhe rankings, impressões e cliques orgânicos"
        actions={
